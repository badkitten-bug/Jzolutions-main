  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        const buildLogger = logger.fork("Jzolutions");

        const virtualModuleId = 'Jzolutions:config';
        ...
        updateConfig({
          ...
          vite: {
            plugins: [
              {
                name: 'vite-plugin-Jzolutions-config',
                resolveId(id) {
        ...
        if (typeof _themeConfig === "string") {
          ...
          buildLogger.info(`Jzolutions \`${_themeConfig}\` has been loaded.`)
        } else {
          buildLogger.info(`Jzolutions config has been loaded.`)
        }
      },
